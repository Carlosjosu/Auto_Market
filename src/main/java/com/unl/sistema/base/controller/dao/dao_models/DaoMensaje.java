...
<<<<<<< HEAD
        try {
            // Validaciones
            if (mensaje == null) {
                throw new Exception("Mensaje no puede ser null");
            }

            if (mensaje.getContenido() == null || mensaje.getContenido().trim().isEmpty()) {
                throw new Exception("Contenido del mensaje no puede estar vacío");
            }

            if (mensaje.getIdConversacion() == null || mensaje.getIdRemitente() == null) {
                throw new Exception("ID de conversación e ID de remitente son requeridos");
            }
            ...
            // Asegurar que el campo leido esté establecido
            mensaje.setLeido(false); // Los mensajes nuevos están sin leer
            ...
            System.out.println("   Conversación: " + mensaje.getIdConversacion());
            System.out.println("   Remitente: " + mensaje.getIdRemitente());
            System.out.println("   Contenido: " + mensaje.getContenido());
            System.out.println("   Fecha: " + mensaje.getFechaEnvio());

        } catch (Exception e) {
            System.err.println("❌ Error en addMensaje: " + e.getMessage());
            e.printStackTrace();
